...
class TaskManager:
    ...
    def get_tasks(self, filter_type: str  = None, value: str | int | None = None) -> List[Task]|Task:
        if filter_type is None:
            return self.__tasks
            
        match filter_type:
            case 'id':
                return next((task for task in self.__tasks if task.id == value), None)
            case 'category':
                return [task for task in self.__tasks if value.lower() in task.category.lower()]
            case 'title':
                return [task for task in self.__tasks if value.lower() in task.title.lower()]
            case 'description':
                return [task for task in self.__tasks if value.lower() in task.description.lower()]
            case 'due_date':
                return [task for task in self.__tasks if value in task.due_date]
            case 'priority':
                return [task for task in self.__tasks if value.lower() in task.priority.lower()]
            case 'status':
                return [task for task in self.__tasks if value.lower() == task.status.lower()]
