    ...
    while True:
        ...
        if value:
            try:
                ...
                print(
                    f"Невозможно преобразовать '{value}' в {expected_type.__name__}"
                )
        else:
...
def main(file_path_to_json="db/tasks.json"):
    ...
    while True:
        ...
        # View tasks
            ...
            tasks_filter = input(
                "Введите название категории для фильтрации (или оставьте пустым для просмотра всех задач): "
            )
            if tasks_filter:
        ...
        # Add task
            ...
            due_date = validator(
                "Введите срок (в формате ГГГГ-ММ-ДД): ", format="date"
            )
            priority = validator(
        ...
        # Edit task
            ...
            if task:
                ...
                print(
                    f"6. Изменить статус выполнения(сейчас -> {task.status})"
                )
                print("-" * 50)
                print("0. Вернуться в главное меню\n")
                ...
                if edit_choice == "1":
                    ...
                    task.due_date = validator(
                        "Новый срок (YYYY-MM-DD): ", format="date"
                    )
                elif edit_choice == "5":
                    task.priority = validator(
                        "Новый приоритет: ", format="priority"
                    )
                elif edit_choice == "6":
        ...
        # Delete task by ID,NAME,CATEGORY
            ...
            if delete_type == 0:
                ...
                task_category = validator(
                    "Введите категорию задачи для удаления: ", str
                )
